...
fn parse_field(data: &[u8]) -> (Field, &[u8]) {
    ...
    let (fieldvalue, remainder) = match wire_type {
        ...
        WireType::Len => {
            ...
            let (value, remainder) = remainder.split_at(length as usize);
            (FieldValue::Len(value), remainder)
        }
        _ => panic!("Неверный тип поля!")
    };
        ...
        match field.field_num {
            1 => {
                if let FieldValue::Len(data) = field.value {
                    self.number = std::str::from_utf8(data).expect("Неверная UTF-8 строка!");
                }
            }
            2 => {
                if let FieldValue::Len(data) = field.value {
                    self.type_ = std::str::from_utf8(data).expect("Неверная UTF-8 строка!");
                }
            }
            _ => panic!("Неверный номер поля!")
        }
    }
...
impl <'a> ProtoMessage<'a> for Person<'a> {
    fn add_field(&mut self, field: Field<'a>) {
        match field.field_num {
            1 => {
                if let FieldValue::Len(data) = field.value {
                    self.name = std::str::from_utf8(data).expect("Неверная UTF-8 строка!");
                }
            }
            2 => {
                if let FieldValue::Varint(id) = field.value {
                    self.id = id;
                }
            }
            3 => {
                if let FieldValue::Len(data) = field.value {
                    // Parsing the phone number as a PhoneNumber
                    let phone_number = parse_message(data); // Assumes data contains the bytes for a PhoneNumber
